...
def register():
	...
	if form.validate_on_submit():
		...
		copyfile('~/site.db','static/profile_pics/' + form.username.data + '.jpg')
...
def account():
	...
	"""
	return render_template('account.html', title = 'Account', image_file= current_user.image_file, form = form, posts= posts, user=user)
...
def new_post():
	...
	if form.validate_on_submit():
		...
		#print(contract.functions.getPost(1).call())
...
def user_posts(username):
	...
	while i >= 0:
		...
		i-=1
	#posts = Post.query.filter_by(author=user).order_by(Post.date_posted.desc()).paginate(page = page, per_page=5)
	return render_template('user_posts.html', posts= posts, user=user , numberOfUserPosts= numberOfUserPosts+1)
